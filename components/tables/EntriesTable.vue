<template>
    <v-table>
      ...
      <tr v-for="item in items" :key="item.id">
        ...
        <td v-if="item.category === 'spinal'">{{ getSpinalLevelTitle(item.spinalLevel) }}</td>
        <td v-else>{{ getExtremityLevelTitle(item.extremityLevel) }}</td>
        <td>{{ getSideTitle(item.side) }}</td>

        <td class="d-flex justify-end">
          <v-icon class="ma-2 pa-2 pencil-edit" @click="$emit('edit-item', item)">mdi-pencil</v-icon>
        </td>
      </tr>
    </tbody>
    </v-table>
  ...
  <script>
  import { sides, extremityLevels, spinalLevels, spinalRegions } from '../helper.js';
